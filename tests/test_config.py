...
def test_llm_config(sample_config):
    ...
    assert llm_config.deployments[2].ssh.hostname == "remote_host"
    assert llm_config.selection_strategy == "priority"

def test_model_abstraction_config(sample_config):
    ...
    assert len(model_config.model_config) == 1
    assert model_config.model_config[0].name == "primary"
    assert model_config.model_config[0].quantization is True

def test_rag_config(sample_config):
...
def test_kubernetes_config(sample_config):
...
def test_full_config(sample_config):
    config = PyDiagnoConfig(**sample_config)
